...
static void read_translations(FILE *file, joint_t *joint) {
	...
	int items = fscanf(file, " translations %d:", &joint->num_translations);
    ...
    if (joint->num_translations > 0) {
        ...
        for (int i = 0; i < joint->num_translations; i++) {
...
static void read_rotations(FILE *file, joint_t *joint) {
	int items = fscanf(file, " rotations %d:", &joint->num_rotations);
    ...
    if (joint->num_rotations > 0) {
        ...
        for (int i = 0; i < joint->num_rotations; i++) {
...
static void read_scales(FILE *file, joint_t *joint) {
	...
	int items = fscanf(file, " scales %d:", &joint->num_scales);
    ...
    if (joint->num_scales > 0) {
        ...
        for (int i = 0; i < joint->num_scales; i++) {
            items = fscanf(file, " time: %f, value: [%f, %f, %f]",
                           &joint->scale_times[i],
...
static joint_t load_joint(FILE *file) {
	...
	int items = fscanf(file, " joint %d:", &joint.joint_index);
    assert(items == 1);
